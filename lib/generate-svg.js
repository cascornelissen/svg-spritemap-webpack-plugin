...
module.exports = (files = [], options = {}) => {
    ...
    files.forEach((file) => {
        ...
        if ( options.generate.symbol ) {
            ...
            symbol.setAttribute('id', `${formatPrefix(options.generate.symbol)}${validId}`);
        ...
        if ( options.generate.use ) {
            ...
            use.setAttribute('xlink:href', `#${formatPrefix(options.generate.symbol)}${validId}`);
        ...
        if ( options.generate.view ) {
            ...
            view.setAttribute('id', `${formatPrefix(options.generate.view)}${validId}`);
            view.setAttribute('viewBox', `0 ${Math.max(0, y - (options.gutter / 2))} ${width + (options.gutter / 2)} ${y + height + (options.gutter / 2)}`);
