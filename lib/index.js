...
module.exports = class SVGSpritemapPlugin {
    ...
    apply(compiler) {
        ...
        compiler.hooks.make.tap(plugin, (compilation) => {
            ...
            switch ( stylesType ) {
                case 'dir':
                    return fs.writeFileSync(this.options.styles, styles, 'utf8');
                case 'module':
                    return fs.writeFileSync(path.resolve(__dirname, '../', this.options.styles.replace('~', '')), styles, 'utf8');
            }
    ...
    getStylesType(styles, option = '') {
        if ( !styles ) {
            return;
        }

        if ( path.parse(option).dir ) {
            return 'dir';
        }
