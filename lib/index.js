...
const { merge } = require('./helpers/svgo-merge');
            ...
            Promise.all(this.files.map((file) => {
                ...
                generateSVG(sources, {
                    ...
                    compilation.hooks.optimizeAssets.tapAsync(plugin, (assets, callback) => {
                        ...
                        const svgoConfig = merge(outputOptions.svgo, {
                            plugins: [{
                                name: 'removeTitle',
                        ...
                        svgoConfig.plugins = svgo.extendDefaultPlugins(svgoConfig.plugins);

                        const output = svgo.optimize(assets[this.filenames.spritemap].source(), svgoConfig);
