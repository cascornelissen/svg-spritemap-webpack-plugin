...
module.exports = class SVGSpritemapPlugin {
    ...
    apply(compiler) {
        ...
        compiler.hooks.entryOption.tap(plugin, (context, entry) => {
            ...
            const helperFile = fs.readFileSync(path.join(__dirname, '/helpers/svg4everybody.template.js'), 'utf8');
            const helperPath = path.join(__dirname, '/../svg4everybody-helper.js');
            fs.writeFileSync(helperPath, helperFile.replace('{/* PLACEHOLDER */}', JSON.stringify(options.svg4everybody)), 'utf8');
            ...
            if ( typeof entry === 'string' ) {
                applyEntryPlugin([entry, helperPath]);
            } else if ( Array.isArray(entry) ) {
                if ( !entry.includes(helperPath) ) {
                    applyEntryPlugin([...entry, helperPath]);
                }
            } else if ( isPlainObject(entry) ) {
                Object.keys(entry).forEach((name) => {
                    if ( typeof entry[name] === 'string' ) {
                        applyEntryPlugin([entry[name], helperPath], name);
                    } else if ( Array.isArray(entry[name]) ) {
                        if ( !entry[name].includes(helperPath) ) {
                            applyEntryPlugin([...entry[name], helperPath], name);
                        }
                    } else {
