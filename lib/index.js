...
import RawModule from 'webpack/lib/RawModule.js';
import intersection from 'lodash/intersection.js';
const { RawSource } = webpack.sources || import('webpack-sources'); // fallback to 'webpack-sources' if webpack.sources is undefined
import SingleEntryPlugin from 'webpack/lib/SingleEntryPlugin.js';

// Helpers
import formatOptions from './options-formatter.js';
import generateSVG from './generate-svg.js';
import generateStyles from './generate-styles.js';
import generateSVGOConfig from './helpers/generate-svgo-config.js';
import variableParser from './variable-parser.js';

// Errors & Warnings
import { OptionsMismatchWarning, VariablesNotSupportedInLanguageWarning, VariablesNotSupportedWithFragmentsWarning, NoSourceFilesWarning } from './errors/index.js';
...
export default class SVGSpritemapPlugin {
    ...
    apply(compiler) {
        ...
        compiler.hooks.make.tap(plugin, (compilation) => {
            ...
            compilation.hooks.processAssets.tap({
                ...
                const stripped = variableParser.stripVariables(asset.source);
    ...
    generateStyles(compilation) {
        ...
        if ( !['scss', 'sass'].includes(extension) && variableParser.findVariables(this.spritemap).length ) {
        ...
        if ( stylesOptions.format === 'fragment' && variableParser.findVariables(this.spritemap).length ) {
