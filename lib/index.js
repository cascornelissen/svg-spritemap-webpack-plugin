    ...
    apply(compiler) {
        ...
        compiler.hooks.make.tapAsync(plugin, (compilation, callback) => {
            ...
            generateSVG(this.files, {
                ...
                compilation.hooks.afterOptimizeTree.tap(plugin, () => {
                    ...
                    if (WEBPACK_VERSION.startsWith('4.')) {
                        module.hash = '';
                        sourceChunk.addModule(module);
                    } else {
                        // TODO
                        // compilation.addRuntimeModule(sourceChunk, module);
                        // compilation.chunkGraph.connectChunkAndModule(sourceChunk, module);
                    }
                })
                ...
                compilation.hooks.optimizeAssets.tapAsync(plugin, (assets, callback) => {
                    ...
                    const files = Array.from(sourceChunk.files).map((filename) => {
                    ...
                    if (WEBPACK_VERSION.startsWith('4.')) {
                        sourceChunk.files = files;
                    } else {
                        sourceChunk.files = new Set(files);
                    }
                    ...
                    compiler.hooks.thisCompilation.tap(plugin, (compilation) => {
                        compilation.hooks.processAssets.tap({
                            ...
                            compilation.chunks.forEach((chunk) => {
                                ...
                                chunk.files.add(outputOptions.filename);
                            });
                        });
                    });
                }
