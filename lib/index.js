...
module.exports = class SVGSpritemapPlugin {
    ...
    rewriteAssetHash(filename, assets = [], hashes = []) {
        // Update regex's in original filename with hash values accordingly
        const hashedFilename = hashes.reduce((output, hash) => {
            const regex = new RegExp(hash.regex, 'ig');
            return output.replace(regex, hash.value);
        }, filename);
    ...
    formatCSS(symbols = []) {
        ...
        // TODO: Rewrite <symbol> to <svg>
        const styles = symbols.map((symbol) => {
            const selector = symbol.getAttribute('id');
            const dataURI = svgToMiniDataURI(XMLSerializer.serializeToString(symbol));
