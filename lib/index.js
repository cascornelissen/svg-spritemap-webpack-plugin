...
module.exports = class SVGSpritemapPlugin {
    ...
    writeStylesToDisk(styles, type) {
        ...
        const locationIsExist = fs.existsSync(location);

        // Make sure we don't rewrite the file when it's correct already
        const contents = locationIsExist && fs.readFileSync(location, 'utf8');
        if ( contents === styles ) {
            return;
        }

        // if location type is 'dir'
        // check if directories path to location is exist
        // and create its if not
        if (type === 'dir' && !locationIsExist) {
            let locationDirname = path.dirname(location);
            if (!fs.existsSync(locationDirname)) {
                mkdirp.sync(locationDirname);
            }
        }
