...
module.exports = class SVGSpritemapPlugin {
    ...
    apply(compiler) {
        ...
        compiler.hooks.make.tap(plugin, (compilation) => {
            ...
            // Generate styles
            const styles = this.generateStyles(spritemap, path.extname(this.options.styles).substring(1).toLowerCase());
            ...
            switch ( stylesType ) {
                case 'dir':
                    fs.writeFileSync(this.options.styles, styles, 'utf8');
                    break;
                case 'module':
                    fs.writeFileSync(path.resolve(__dirname, '../', this.options.styles.replace('~', '')), styles, 'utf8');
                    break;
            }
            ...
            compilation.hooks.optimizeAssets.tapAsync(plugin, (assets, callback) => {
                ...
                const hashedAssets = this.rewriteAssetsHashes(this.options.filename, compilation.assets, [{
                    pattern: /\[hash]/,
            ...
            compilation.hooks.optimizeAssets.tapAsync(plugin, (assets, callback) => {
                ...
                const hashedAssets = this.rewriteAssetsHashes(this.options.styles, compilation.assets, [{
                    pattern: /\[hash]/,
                    value: compilation.getStats().hash
                }, {
                    pattern: /\[contenthash]/,
    ...
    generateStyles(spritemap, extension) {
        ...
        // Execute formatter
        const formatter = StyleFormatters[extension.toUpperCase()];
